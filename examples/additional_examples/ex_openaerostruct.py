...
cruise_speed = csdl.Variable(value=40.)
...
# Define Breguet Range Fuel Burn constants
R_range = csdl.Variable(value=1000.0 * 1000.0)  # 1000 km in meters
SFC = csdl.Variable(value=17.0e-6)  # 17 mg/Ns
W_0 = csdl.Variable(value=3000.0)   # 3000 N empty weight + payload
W_struct = structural_mass * 9.81
W_total = W_0 + W_struct

# Compute L/D (using Trefftz drag)
L_D = L / (CDi * 0.5 * rho_array[0] * (cruise_speed**2) * planform_area + 1e-6)

# Breguet Range equation for Fuel Weight
W_fuel = W_total * (csdl.exp((R_range * SFC) / (cruise_speed * L_D)) - 1.0)
objective = W_fuel
objective.set_as_objective(scaler=1.e-1)

# L = W constraint (trim condition)
(L - (W_total + W_fuel)).set_as_constraint(equals=0.0, scaler=1.e-3)

# Structural failure constraint (max tip deflection)
tip_deflection = csdl.norm(beam_displacement[0, 0:3])
tip_deflection.set_as_constraint(upper=0.5, scaler=1.0)
...
jax_sim = csdl.experimental.JaxSimulator(
    ...
    additional_outputs=[Di, L, CL, CDi, Cp, panel_mesh, planform_area, aspect_ratio_calc, structural_mass, beam_displacement, W_fuel, tip_deflection, W_total] + geometry_coefficients,
    gpu=False
)
