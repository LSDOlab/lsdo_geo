...
wingspan_outer_dv = csdl.Variable(shape=(1,), value=wingspan.value)
root_chord_outer_dv = csdl.Variable(shape=(1,), value=root_chord.value)
tip_chord_outer_dv = csdl.Variable(shape=(1,), value=tip_chord_left.value)
sweep_angle_outer_dv = csdl.Variable(shape=(1,), value=sweep_angle_left.value)
...
area = 2 * 0.5 * wingspan * (root_chord + tip_chord_left)/2

leading_edge_right_computed = geometry.evaluate(leading_edge_right, plot=False)
trailing_edge_right_computed = geometry.evaluate(trailing_edge_right, plot=False)
leading_edge_root_computed = geometry.evaluate(leading_edge_center, plot=False)
trailing_edge_root_computed = geometry.evaluate(trailing_edge_center, plot=False)

leading_edge_right_length = csdl.norm(leading_edge_right_computed - leading_edge_root_computed)
trailing_edge_right_length = csdl.norm(trailing_edge_right_computed - trailing_edge_root_computed)
half_perimeter = leading_edge_right_length + trailing_edge_right_length + tip_chord_right
perimeter = 2 * half_perimeter

objective = -area

objective.set_as_objective()
perimeter.set_as_constraint(equals=40.0)

wingspan_outer_dv.set_as_design_variable(lower=0.01, upper=100.0)
root_chord_outer_dv.set_as_design_variable(lower=0.01, upper=100.0)
tip_chord_outer_dv.set_as_design_variable(lower=0.01, upper=100.0)
sweep_angle_outer_dv.set_as_design_variable(lower=0.0, upper=np.pi/3)
...
optimization_problem = modopt.CSDLAlphaProblem(problem_name='rectangular_wing_geometric_optimization', simulator=sim)

optimizer = modopt.PySLSQP(optimization_problem, solver_options={'maxiter':100, 'acc':1.e-5}, readable_outputs=['x'])
