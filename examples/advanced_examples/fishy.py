...
# grid_points = np.array(grid.points).reshape(grid_shape)

# import vedo
# vedo.show(vedo.Points(grid_points.reshape((-1,3))), interactive=True)



num_coefficients_u = 51
num_coefficients_v = 21

# fish_surface = bsp.fit_b_spline(fitting_points=grid_points, order=(2,3), 
...
quad_mesh = fish_surface.evaluate(volume_elements_mesh_parametetric_coordinates).reshape((N_L*2,N_R,3))
...
fishy = lsdo_geo.Geometry(name='fishy', space=fisho_b_spline_set.space, coefficients=fisho_b_spline_set.coefficients, 
                          num_physical_dimensions={'fisho_volume': 3})
