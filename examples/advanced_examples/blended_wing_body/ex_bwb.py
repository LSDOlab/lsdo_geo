...
video = vedo.Video(name="examples/advanced_examples/blended_wing_body/bwb_geometric_variables.mp4", fps=11, backend='opencv')
...
exit()
# geometry_solver, geometric_variables, ffd_blocks = setup_geometry(geometry, geometry_values_dict, num_wing_DVs)
# returning ffd blocks for plotting & debugging


#endregion
# plotting mesh for debugging the geometry
if False:
    # left_wing_ffd_block = ffd_blocks[-2]
    # plotting_elements = left_wing_ffd_block.plot(show=False)
    # left_wing.plot(additional_plotting_elements=plotting_elements)
    ...
    plot_pressure_distribution(panel_mesh.value, TE_coloring, connectivity=triangles, interactive=True, top_view=False, cmap='rainbow')
    # exit()

recorder.inline = not shutdown_inline

if True:
    print('running inner optimization')
    start_inner_opt = time.time()
    geometry_solver.evaluate(geometric_variables)
    end_inner_opt = time.time()
    inner_opt_time = end_inner_opt - start_inner_opt
    print(f'inner optimization run time: {inner_opt_time} seconds')
