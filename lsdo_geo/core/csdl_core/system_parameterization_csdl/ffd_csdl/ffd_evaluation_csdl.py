...
class FFDEvaluationCSDL(csdl.Model):
    ...
    def define(self):
        ...
        # Declare input
        ffd_coefficients = self.declare_variable('ffd_coefficients', val=ffd_set.coefficients)
        ...
        if ffd_set.num_dof != 0:
            embedded_entities = csdl.sparsematmat(ffd_coefficients, sparse_mat=embedded_entities_map)
...
if __name__ == "__main__":
    ...
    plotting_elements = wing_ffd_block.plot_sections(coefficients=ffd_coefficients.reshape(wing_ffd_b_spline_volume.shape), plot_embedded_entities=False, opacity=0.75, show=False)
    ...
    plotting_elements = wing_ffd_block.plot_sections(
        coefficients=(ffd_coefficients[:wing_ffd_block.num_coefficients,:]).reshape(wing_ffd_b_spline_volume.shape),
