...
cdef get_basis_volume_matrix(
    ...
    for i_pt in range(num_points):
        ...
        i_start_w = get_basis_w(order_w, num_coefficients_w, w_vec[i_pt], knot_vector_w, basis_w)
            ...
            for i_order_v in range(order_v):
                for i_order_w in range(order_w):
                   ...
                   # print('basis_u: ', basis_u[i_order_u])
                    ...
                    col_indices[i_nz] = num_coefficients_w * (num_coefficients_v * (i_start_u + i_order_u)   +   (i_start_v + i_order_v) ) + (i_start_w + i_order_w)
